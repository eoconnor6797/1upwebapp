...
app
  ...
  .then(() => {
    ...
    server.use(auth.passwordless.acceptToken({ successRedirect: '/dashboard' }))

    server.post('/sendtoken',
      auth.passwordless.requestToken((email, delivery, fn) =>
      fn(null, email)), (req, res) =>
      res.json('ok')
    )
